...
result that may not exist. btclib-node vendors no submodule, so this is
a no-op here.
...
def component(readings: list[Reading]) -> dict[str, Any]:
    ...
    for entry in readings:
        ...
        properties.append({"name": "btclib-node:requires-dist", "value": entry.line})
        if entry.extras is not None:
            properties.append({"name": "btclib-node:extras", "value": entry.extras})
...
def submodule_component(path: str, url: str, sha: str) -> dict[str, Any]:
    ...
    return {
        ...
        "properties": [{"name": "btclib-node:submodule-path", "value": path}],
    }
...
def submodule_components(repo_root: Path) -> list[dict[str, Any]]:
    ...
    -- btclib-node itself -- makes this a no-op.
    """
...
def build_sbom(wheel: Path, sdist: Path, epoch: int, repo_root: Path) -> dict[str, Any]:
    ...
    if requires_python is not None:
        root["properties"] = [
            {"name": "btclib-node:requires-python", "value": requires_python}
        ]
