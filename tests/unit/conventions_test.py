...
usable: a repository needs the ones its own prose states, plus the
public surface, which a repository publishing an importable package has
regardless. That clause is right, and its price is that an *absent*
...
# section 7's own list, in its order and its words: the lead of each
...
# backtick excludes: `| --- | --- |` has no backtick to match. The module
# column names a path relative to tests/, not to this file, because a
# convention test need not live beside this one.
_ROW = re.compile(
...
def test_every_module_named_exists(convention: str, module: str) -> None:
    ...
    assert (_TESTS / module).is_file(), (
        ...
        " which is not a file under tests/"
    )
